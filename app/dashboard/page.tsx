...
function mapDbReviewToCard(row: DbReview): ReviewWithResponse {
  const resp = row.responses?.[0] ?? null;
  const authorName = row.customer_name ?? "Cliente";
  return {
    id: row.id,
    authorName,
...
function buildMetrics(reviews: DbReview[]): DashboardMetrics {
  const now = Date.now();
  ...
  // Rating trend: last 30d vs previous 30d
  ...
  const recentAvg = avg(recent30.map((r) => r.rating));
  const prevAvg = avg(prev30.map((r) => r.rating));
  const ratingDiff =
  ...
  const avgRating: Metric = {
    ...
    change:
      ratingDiff !== null
        ...
        : "Sin datos",
    trend:
      ratingDiff === null ? "neutral" : ratingDiff > 0 ? "positive" : ratingDiff < 0 ? "negative" : "neutral",
    changeContext: "",
  };
  ...
  const pendingCount: Metric = {
    ...
    change: urgent.length > 0 ? `${urgent.length} urgentes` : "Al día",
    trend: urgent.length > 0 ? "negative" : "neutral",
    changeContext: "",
  };

  // Avg response time (hours) over last 30d approved responses
  ...
  const avgResponseTime: Metric = {
    ...
    change: avgHours !== null ? "" : "Sin datos",
    trend: "neutral",
    changeContext: "",
  };
...
function buildChartData(reviews: DbReview[]): ChartDataPoint[] {
  ...
  // Pre-fill all 30 days so the axis is stable
  for (let i = 29; i >= 0; i--) {
    const d = new Date(now - i * 24 * 3600 * 1000);
    const key = `${d.getDate()} ${d.toLocaleString("es", { month: "short" })}`;
    map.set(key, []);
  }
...
function buildAlerts(reviews: DbReview[]): SmartAlert[] {
  const now = Date.now();
  ...
  // Negative word frequency in last 7 days
  ...
  for (const [word, count] of wordCounts) {
    if (count >= 3) {
      alerts.push({
        id: `word-${word}`,
        type: "warning",
        message: `${count} reseñas recientes mencionan "${word}"`,
      });
    }
  }
  ...
  if (recentAvg !== null && prevAvg !== null) {
    ...
    if (Math.abs(diff) >= 0.3) {
      alerts.push({
        ...
        message:
          diff > 0
            ? `Tu rating subió ${diff.toFixed(1)} puntos vs el mes pasado`
            : `Tu rating bajó ${Math.abs(diff).toFixed(1)} puntos vs el mes pasado`,
      });
    }
  }

  // Critical reviews (rating ≤ 2) in last 48h
    ...
    alerts.push({
      ...
      message:
        critical48h.length === 1
          ? "Nueva reseña de 1-2 ★ en las últimas 48h — requiere atención"
          : `${critical48h.length} reseñas de 1-2 ★ en las últimas 48h — requieren atención`,
    });
  }
  ...
  return (
    <div className="lg:col-span-3 bg-paper border border-line rounded-2xl p-10 text-center">
      ...
      <p className="text-sm text-muted mb-5">
        Añade tu primera reseña para empezar a ver datos aquí.
      </p>
...
// ─── Zero-value metrics for empty/error state ─────────────────────────────────
...
export default function DashboardPage() {
  ...
  const [pendingReviews, setPendingReviews] = useState<ReviewWithResponse[]>([]);
  ...
  const loadData = useCallback(async (businessId: string) => {
    ...
    try {
      ...
      const [allResult, pendingResult] = await Promise.all([
        // All reviews with response timestamps for KPI + chart + alerts
        supabase
          .from("reviews")
          .select(
        ...
        // Pending reviews (full) for the dashboard list
        supabase
          .from("reviews")
          .select("*, responses(*)")
          .eq("business_id", businessId)
      ...
      const allReviews = (allResult.data ?? []) as DbReview[];
      const pendingRaw = (pendingResult.data ?? []) as DbReview[];

      setHasAnyReviews(allReviews.length > 0);
      setMetrics(buildMetrics(allReviews));
      setChartData(buildChartData(allReviews));
      setAlerts(buildAlerts(allReviews));
      setPendingReviews(pendingRaw.map(mapDbReviewToCard));
    } catch (err) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      ...
      {/* KPI grid — always rendered (skeleton or real) */}
