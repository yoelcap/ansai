        ...
        <div className="grid md:grid-cols-3 gap-6 mt-14">
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="display-serif text-2xl font-medium mb-2">{plan.name}</div>
              <p className={cn("text-sm mb-8", plan.featured ? "text-white/70" : "text-muted")}>
                {plan.desc}
              </p>

              <div className="display-serif text-[56px] font-normal leading-none mb-1">
                {plan.price}
                <span className={cn("text-lg font-sans", plan.featured ? "text-white/70" : "text-muted")}>
                  /mes
                </span>
              </div>
              <p className={cn("text-[13px] mb-6", plan.featured ? "text-white/70" : "text-muted")}>
                {t("pricing.period")}
              </p>
              ...
              <div
                className={cn(
                  ...
                  plan.featured
                    ? "bg-white/10"
                    : "bg-forest/[0.05] border border-forest/15"
                )}
              >
                <p
                  className={cn(
                    "text-[10px] font-semibold uppercase tracking-[0.14em] mb-1",
                    plan.featured ? "text-gold" : "text-terra"
                  )}
                >
                  ✦ IA
                </p>
              ...
              <button
                className={cn(
                  "w-full py-3.5 rounded-full text-sm font-semibold border transition-colors",
                  plan.featured
